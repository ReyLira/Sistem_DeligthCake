...
public class EmailV extends Conexion implements Validator {
    ...
    public static boolean duplicadoEmailCliente(String email) {
        try {
       ...
       public static boolean duplicadoEmailEmpleado(String email) {
        try {
            PreparedStatement ps = Conexion.conectar().prepareStatement("SELECT EMAEMP FROM EMPLEADO WHERE EMAEMP = '" + email + "'");
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                return true;
            }
